...
JQL_LVL2_FOR_PCI_ROOT = ['project = LVL2', 'type = "New Feature"']
...
def reccursive_build_tree(jira, ancestor, key, s_squad, b_skip_done):
    ...
    if type == 'Epic':
        if s_squad == 'Network' and not ovhissue.is_network():
            return
    if ovhissue:
        if b_skip_done and proj == 'PCI' and ovhissue.is_closed():
            return
        node = ancestor.add(ovhissue)
        for childkey in ovhissue.childs:
            reccursive_build_tree(jira, node, childkey, s_squad, b_skip_done)
...
class LSD:
    PROJECT = 'LVL2'
    def __init__(self, jira, s_year, s_quarter, s_squad, b_skip_done):
        self._jira = jira
        self.year = s_year
    ...
    def build_tree(self):
        ...
        for key in l_new_features:
            reccursive_build_tree(self._jira, self.tree, key, self.s_squad, self.b_skip_done)
